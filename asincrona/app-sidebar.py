...
st.write(f"Number of Records With Fare Between {fare_min} and {fare_max}: {subset_fare.shape[0]} regs")

# Display of the dataset whith the max fare selected
#st.write(titanic_data.query(f"""fare>=@fare_max and fare<=@fare_min"""))
st.write(titanic_data[(titanic_data["fare"] >= fare_min) & (titanic_data["fare"] <= fare_max)])